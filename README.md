...
支持浏览器识别为 `standard` 的手柄，包括合并显示为 `Joy-Con L+R` 的左右 Joy-Con。连接后点击游戏页面并按手柄按键，右侧会显示连接状态及当前机械的键位。单只或未合并的非标准 Joy-Con 暂不支持；选机械、选场景和弹窗仍使用鼠标。
...
运行 `node tests/gamepad-browser.mjs` 可用模拟标准手柄验证浏览器输入，默认访问 5173 端口，可用 `TEST_URL` 指定地址。模拟测试不能替代真实蓝牙手柄验证。
