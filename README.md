...
支持浏览器识别为 `standard` 的手柄，包括合并显示为 `Joy-Con L+R` 的左右 Joy-Con。进入页面自动检测，识别后将操作按钮、镜头提示和操作指南切换为手柄键位，断连后恢复键鼠提示。浏览器首次可能需要点击页面并按一下手柄才会暴露设备。单只或未合并的非标准 Joy-Con 暂不支持。

展厅使用方向键切换机械，A 进入场景选择；弹窗使用方向键或左摇杆选择，A 确认、B 返回，高亮边框标识当前选项。展厅双摇杆仍可试操机械。施工中按 B 或 ＋（Menu）打开暂停菜单，可继续、切换场景、退出到展厅、重开、查看指南、切换声音；塔吊还可选择料场/楼顶视角及将吊物放回原位。菜单也有鼠标入口，断连后仍可关闭。切换场景、重开或返回展厅沿用原有重置本次任务的行为。

| 输入 | 挖掘机 | 汽车起重机 | 塔吊 |
| --- | --- | --- | --- |
| 左摇杆左右 | 上车回转 | 平台回转 | 回转 |
| 左摇杆上下 | 斗杆伸出 / 收回 | 伸臂 / 缩臂 | 小车向外 / 向内 |
| 右摇杆上下 | 动臂抬起 / 放下 | 抬臂 / 放臂 | 吊钩上升 / 下降 |
| 右摇杆左右 | 收斗 / 翻斗 | 吊钩上升 / 下降 | 无 |
| L / R（LB / RB） | 底盘左转 / 右转 | 无 | 无 |
| ZL / ZR（LT / RT） | 后退 / 前进 | 无 | 无 |
| A | 贴地铲装，仅城市工地 | 抓取 / 释放 | 抓取 / 释放 |
| ＋（Menu） / B | 打开菜单 | 打开菜单 | 打开菜单 |

按住 **X** 进入镜头模式：左摇杆平移、右摇杆旋转、ZL/ZR 拉远/拉近，按下右摇杆恢复默认视角。镜头模式不输出机械动作；松开 X 后需摇杆回中、松开按键，再继续操作。连接、失焦恢复、断连重连、关闭弹窗后也需要回中。摇杆有 18% 死区，并支持比例调速；Joy-Con 的 ZL/ZR 为数字按键，以固定速度行走和缩放。

Nintendo 手柄按标准布局的物理位置识别 A（右侧）与 X（上侧）；其他标准手柄使用 A（下侧）、X（左侧）。通过第三方软件转换为 Xbox 手柄时按转换后的布局操作。陀螺仪和震动未接入。

运行 `node tests/gamepad-browser.mjs` 可用模拟标准手柄验证机械与镜头输入；`node tests/gamepad-navigation-browser.mjs` 验证自动提示、纯手柄选择机械与场景、切换/重开/退出、断连回退及窄屏导航。默认访问 5173 端口，可用 `TEST_URL` 指定地址。模拟测试不能替代真实蓝牙手柄验证。
