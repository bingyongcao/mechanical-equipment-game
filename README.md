...
基于 Three.js、TypeScript 和 Rapier 的儿童施工机械游戏原型。使用本项目通过 Blender MCP 建立的 EX 200 挖掘机模型。当前可以完整体验认识机械、选择工地、操控挖掘机、物理搬运石头及完成庆祝。
...
铲装时先稍微放低动臂，再向石堆推进，收斗后抬臂。搬到目标区上方后充分翻斗；如果石头没有滑出，可稍微放低动臂改变倾角。石头需要落下、滚动并停稳后才会计数。
...
- 浏览器回归：加载、七个标注、长按/释放、失焦、行走、暂停、场景切换、返回、庆祝、继续探索和 390px 窄屏；检查浏览器异常和请求错误。
...
目前只有挖掘机可以选择，推土机和起重机明确显示为筹备中。地面是固定地形，还不能挖土变形；履带几何尚未加入循环行走动画。底盘及工作装置采用运动学控制，而非整机动力学，未模拟液压负载、重载倾覆、完整自碰撞或真实履带与土壤接触。基础阻挡检查和已通过的搬运回归不能替代任意姿态的工程验证。
