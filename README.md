...
- 机械展厅：默认选择挖掘机；可在挖掘机与汽车起重机之间切换模型，再确认机械并选择对应场景。支持模型环绕、缩放、平移；挖掘机七个、起重机六个中文部件标签、说明及高亮。
...
- 三个独立场景：城市工地搬运 8 块石头；砂料厂挖掘有限沙层，越墙卸入自卸卡车至 80%；河岸使用汽车起重机把货船上的 5 箱物资吊运到指定位置。
...
目前可选择履带式挖掘机和汽车起重机，塔吊仍显示为筹备中。第一场景地面固定；第二场景可挖掘并使沙层变形，采用粗颗粒和有限体积网格混合模拟。履带几何尚未加入循环行走动画。底盘及工作装置采用运动学控制，起重机货物采用稳定挂载，不模拟绳载摆动、液压负载、重载倾覆或完整自碰撞。基础阻挡检查和搬运回归不能替代任意姿态的工程验证。
